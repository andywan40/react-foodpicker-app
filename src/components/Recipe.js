...
function Recipe({ classes, location, history, match }) {
  ...
  const recipe =
    location.recipe || JSON.parse(sessionStorage.getItem("recipeInfo"));
  useEffect(() => {
      ...
      {recipe && (
        <div className={classes.contentContainer}>
          ...
          <div className={classes.section}>
            ...
            <ul>
              {[recipe.cuisine].map(cuisine => (
                <li key={cuisine} className={classes.cuisineText}>
                  {cuisine}
                </li>
              ))}
            </ul>
          ...
          <div className={classes.section}>
            <h4>calories</h4>
            <ul>
              {[recipe.calories].map(calories => (
                <li key={calories}>{calories}</li>
              ))}
            </ul>
          </div>
