...
export const getDishRecipes = async (dish, nextLink) => {
  const url = nextLink === "" ? `https://react-foodpicker-app-backend.herokuapp.com/api/v1/edamam/dish/${dish}/null` :  `https://react-foodpicker-app-backend.herokuapp.com/api/v1/edamam/dish/${dish}/${nextLink}`;
  try {
    const data = await axios.get(url);
    return data.data;
  } catch (e) {
